      ...
      <div className="max-w-6xl mx-auto px-6 relative">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {certificates.map((cert, index) => (
            <div 
              key={index} 
              className="group relative bg-gray-900/50 backdrop-blur-sm rounded-lg overflow-hidden
              ...
              <div className="absolute bottom-0 left-0 right-0 p-6">
                <div className="relative">
                  ...
                  <div className="flex justify-between items-center mb-3">
                    <h3 className="text-2xl font-oswald font-bold text-white">{cert.name}</h3>
                    <span className="text-ctaOrange font-oswald">{cert.year}</span>
                  </div>
                  <p className="text-gray-300 font-roboto text-sm leading-relaxed group-hover:text-white transition-colors duration-300">
                    {cert.description}
                  </p>
                </div>
              </div>
